{{-- resources/views/livewire/room-card.blade.php --}}
<div class="bg-[#FDF9F3]" x-data="{ showToast: false, toastMessage: '' }" 
        ...
        <div class="container mx-auto px-6 md:px-12 py-4">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="relative">
                    ...
                    <i class="fas fa-chevron-down absolute right-3 top-1/2 transform -translate-y-1/2 text-stone-500 text-sm pointer-events-none"></i>
                </div>
            </div>
        </div>
    </div>
    ...
    <section class="py-16 md:py-20">
        <div class="container mx-auto px-6 md:px-12">
            ...
            <div wire:loading.remove class="text-center mb-12">
                ...
                <p class="text-stone-500 mt-3 max-w-2xl mx-auto">Discover our signature suites designed for unforgettable moments.</p>
            </div>
            @else
            <div wire:loading.remove class="mb-8 text-center">
                <h2 class="text-2xl md:text-3xl font-['Cormorant_Garamond'] font-semibold text-stone-800">{{ ucfirst($activeCategory) }} Suites</h2>
                <p class="text-stone-500 mt-2">{{ $categoryDescriptions[$activeCategory] ?? 'Luxurious accommodations for discerning travelers' }}</p>
            </div>
            @endif
            ...
            <div wire:loading.remove class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <div class="relative h-80 overflow-hidden"
                        ...
                        <div class="absolute top-4 left-4 flex gap-2 z-10">
                            @if($room->badge)
                            <span class="bg-amber-700 text-white px-3 py-1 text-xs font-semibold rounded-full">{{ $room->badge }}</span>
                            @endif
                            @if($room->size)
                            <span class="bg-black/60 backdrop-blur text-white px-3 py-1 text-xs font-semibold rounded-full">{{ $room->size }}</span>
                            @endif
                        </div>
                        ...
                        <button wire:click="toggleFavorite({{ $room->id }})" 
                            ...
                            <i class="fas fa-heart {{ in_array($room->id, $favorites) ? 'text-red-500' : 'text-stone-400' }}"></i>
                        </button>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-2xl font-semibold font-['Cormorant_Garamond'] text-stone-800">{{ $room->name }}</h3>
                            <div class="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded">
                                <i class="fas fa-star text-amber-500 text-sm"></i>
                                <span class="text-sm font-semibold text-stone-700">{{ number_format($room->rating, 1) }}</span>
                ...
                <div class="col-span-full text-center py-20">
                    ...
                    <p class="text-stone-500 mt-2">Try adjusting your filters or view all our luxurious accommodations.</p>
        ...
        <div class="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative"
            ...
            <div class="grid md:grid-cols-2 gap-0">
                ...
                <div class="p-6 md:p-8">
                    <h2 class="text-3xl font-semibold font-['Cormorant_Garamond'] text-stone-800">{{ $selectedRoom->name }}</h2>
                    <div class="flex items-center gap-2 mt-2">
                        ...
                        <span class="text-sm text-stone-500">{{ number_format($selectedRoom->rating, 1) }} · {{ $selectedRoom->review_count }} reviews</span>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-stone-200 flex justify-between items-center">
                        <div>
                            <span class="text-3xl font-bold text-amber-800">${{ number_format($selectedRoom->price, 0) }}</span>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
        ...
        <div class="bg-white rounded-xl max-w-md w-full p-6 relative"
            ...
            <button @click="open = false; $wire.closeAmenitiesModal()" class="absolute top-4 right-4 text-stone-400 hover:text-stone-800">
    ...
    <div x-data="{ show: true }"
         x-show="show"
         x-init="setTimeout(() => { show = false; $wire.dispatch('hide-toast'); }, 3000)"
